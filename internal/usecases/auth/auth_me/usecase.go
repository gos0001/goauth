package auth_me

import (
	"context"
	"time"

	postgresadapter "github.com/gos0001/goauth/internal/adapter/postgres"
	"github.com/gos0001/goauth/internal/domain"
)

type Postgres interface {
	GetUserByID(ctx context.Context, id string) (domain.User, error)
}

type Usecase struct {
	postgres Postgres
}

func New(pg *postgresadapter.Adapter) *Usecase {
	return &Usecase{postgres: pg}
}

// Execute reads the caller's own record from the database rather than echoing
// the token's claims. The token was minted up to its whole lifetime ago; the
// row is what is true now.
func (uc *Usecase) Execute(ctx context.Context, in Input) (Output, error) {
	user, err := uc.postgres.GetUserByID(ctx, in.UserID)
	if err != nil {
		return Output{}, err
	}

	return Output{
		ID:                 user.ID,
		Username:           user.Username,
		Email:              user.Email,
		IsAdmin:            user.IsAdmin,
		Status:             string(user.Status),
		MustChangePassword: user.MustChangePassword,
		LastLoginAt:        user.LastLoginAt,
		CreatedAt:          user.CreatedAt,
	}, nil
}

type Input struct {
	UserID string `json:"-"`
}

type Output struct {
	ID                 string     `json:"id"`
	Username           string     `json:"username,omitempty"`
	Email              string     `json:"email,omitempty"`
	IsAdmin            bool       `json:"is_admin"`
	Status             string     `json:"status"`
	MustChangePassword bool       `json:"must_change_password"`
	LastLoginAt        *time.Time `json:"last_login_at,omitempty"`
	CreatedAt          time.Time  `json:"created_at"`
}
