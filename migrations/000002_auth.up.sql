-- goauth core schema: identity, refresh-token sessions, audit trail.
--
...
CREATE INDEX ga_sessions_user_live_idx ON ga_sessions (user_id) WHERE used_at IS NULL;
CREATE INDEX ga_sessions_family_idx    ON ga_sessions (family_id);
CREATE INDEX ga_sessions_expires_idx   ON ga_sessions (expires_at);

CREATE TABLE ga_audit_log (
...
CREATE INDEX ga_audit_log_target_idx  ON ga_audit_log (target_user_id, created_at DESC);
CREATE INDEX ga_audit_log_created_idx ON ga_audit_log (created_at DESC);
