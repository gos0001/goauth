...
const (
	undefinedDatabase = "3D000" // the database in the URL does not exist
...
// New opens the pool and pings it, so a bad URL fails at startup rather than on
// the first request.
//
...
// creates the one named in the URL.
//
...
func createDatabase(ctx context.Context, dsn string) error {
	u, err := url.Parse(dsn)
	if err != nil {
		return fmt.Errorf("postgres: POSTGRES_URL is not a valid URL: %w", err)
	}

	name := strings.TrimPrefix(u.Path, "/")
	if name == "" {
		return errors.New("postgres: POSTGRES_URL names no database")
	}
